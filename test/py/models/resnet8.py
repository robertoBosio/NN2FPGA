...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, weight_bits=8, act_bits=8):
        ...
        self.layer1 = self._make_layer(block, 16, layers[0], act_bits=act_bits, weight_bits=weight_bits)
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2, act_bits=act_bits, weight_bits=weight_bits)
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2, act_bits=act_bits, weight_bits=weight_bits)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, act_bits=8, weight_bits=8):
        downsample = None
        ...
        layers = []
        layers.append(block(inplanes = self.inplanes, planes = planes, stride = stride, downsample = downsample, weight_bits=weight_bits, act_bits=act_bits))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(inplanes = self.inplanes, planes = planes, weight_bits=weight_bits, act_bits=act_bits))
...
def resnet8(num_classes=10, weight_bits=8, act_bits=8, **kwargs):
