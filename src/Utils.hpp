...
template <class t_input, int c_scale, int c_clip, int c_mask, class t_output>
t_output QuantAct(t_input i_data) {
  const t_output c_msb = sizeof(t_output) * 8 - 1;
...
template <class t_input, int c_ich, int c_och, int c_ih, int c_iw, int c_oh,
          int c_ow, int c_fh, int c_fw, int c_str, int c_pad>
void ShiftOp(hls::stream<t_input> &i_data,
...
#pragma HLS pipeline style = frp
        ...
        for (uint8_t s_fh = 0; s_fh < c_fh; s_fh++) {
          for (uint8_t s_fw = 0; s_fw < c_fw; s_fw++) {
            ...
            if (s_index < (c_index - 1)) s_data[s_index].write(s_input);
          }
        }
      }
    }
