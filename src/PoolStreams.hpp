...
void PoolOp(hls::stream<t_input> &i_data, hls::stream<t_output> &o_data) {
  ...
  for (uint8_t s_och = 0; s_och < c_och; s_och++) s_acc_buff[s_och] = c_quant;
    ...
    for (uint8_t s_index = 0; s_index < c_index; s_index++) {
      for (uint8_t s_och = 0; s_och < c_och; s_och++) {
        ...
        if (c_pool == 1) {  // Max Poool
          if (s_input > s_acc_buff[s_och]) s_acc_buff[s_och] = s_input;
        }
      }
    }
  }
...
void PoolOp(hls::stream<t_input_struct> &i_data,
...
template <class t_input, class t_acc, class t_output, int c_ich, int c_och,
          int c_ih, int c_iw, int c_oh, int c_ow, int c_fh, int c_fw, int c_str,
          int c_pad, int c_bypass, int c_pool>
void PoolOp(hls::stream<t_input> i_data[c_fh * c_fw],
    ...
    for (uint8_t s_och = 0; s_och < c_och; s_och++) {
      ...
      for (uint8_t s_index = 0; s_index < c_index; s_index++) {
        ...
        if (c_pool == 0)  // Average Pool
          s_acc_buff += s_input;
        if (c_pool == 1) {  // Max Poool
          if (s_input > s_acc_buff) s_acc_buff = s_input;
        }
      }
...
template <class t_input, class t_acc, class t_output, int c_ich, int c_och,
          int c_ih, int c_iw, int c_oh, int c_ow, int c_fh, int c_fw, int c_str,
          int c_pad, int c_pool>
void PoolKernel(hls::stream<t_input> &i_data, hls::stream<t_output> &o_data) {
#pragma HLS inline
...
#pragma HLS STREAM variable = s_compute depth = 10
    ...
    ShiftOp<t_input, c_ich, c_och, c_ih, c_iw, c_oh, c_ow, c_fh, c_fw, c_str,
...
#pragma HLS unroll
    ...
    PoolOp<t_input, t_acc, t_output, c_ich, c_och, c_ih, c_iw, c_oh, c_ow, c_fh,
...
template <class t_input, class t_output, class t_acc, int c_ich, int c_och,
          int c_iw, int c_ih, int c_ow, int c_oh, int c_fw, int c_fh, int c_str,
          int c_pad,
          int c_pool  // 0 average, 1 max
          >
void PoolStreams(hls::stream<t_input> &i_data, hls::stream<t_output> &o_data) {
#pragma HLS inline

  PoolKernel<t_input, t_acc, t_output, c_ich, c_och, c_ih, c_iw, c_oh, c_ow,
