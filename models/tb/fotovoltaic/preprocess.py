...
def process_image(n_images, onnx_path, dataset):
    ...
    try:
        os.remove("/tmp/results_preprocessed_0.bin")
        os.remove("/tmp/results_preprocessed_1.bin")
        os.remove("/tmp/results_preprocessed_2.bin")
    except FileNotFoundError:
        pass
    ...
    with torch.no_grad():
        with open("/tmp/images_preprocessed.bin", "ab") as f_image, \
             ...
             open("/tmp/results_preprocessed_0.bin", "ab") as f_res0, \
             open("/tmp/results_preprocessed_1.bin", "ab") as f_res1, \
             open("/tmp/results_preprocessed_2.bin", "ab") as f_res2, \
             open("/tmp/results_preprocessed0_txt.txt", "w") as f_res_onnx_txt:
            ...
            for images, labels in eval_dataset:
                ...
                output0 = outputs['global_out']
                output1 = outputs['global_out_1']
                output2 = outputs['global_out_2']
                output0 = np.squeeze(output0)  # shape: (18, 80, 80)
                output1 = np.squeeze(output1)  # shape: (18, 40, 40)
                output2 = np.squeeze(output2)  # shape: (18, 20, 20)
                assert output0.shape == (18, 80, 80)
                assert output1.shape == (18, 40, 40)
                assert output2.shape == (18, 20, 20)
                output0 = np.transpose(output0, (1, 2, 0))
                output1 = np.transpose(output1, (1, 2, 0)) 
                output2 = np.transpose(output2, (1, 2, 0)) 
                f_res0.write((output0.flatten().astype(np.float32)).tobytes())
                f_res1.write((output1.flatten().astype(np.float32)).tobytes())
                f_res2.write((output2.flatten().astype(np.float32)).tobytes())
