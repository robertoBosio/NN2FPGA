...
class ConsumeStream(CustomOp):
    ...
    def append_top_outputs(self, function: cpp_function) -> cpp_function:
        """
        Append the input streams to the top function.
        """

        var = cpp_variable(self.get_stream_name(self.onnx_node.output[0]), f"hls::stream<ap_axiu<{self.get_nodeattr('axi_bitwidth')}, 0, 0, 0>>&")
        function.add_argument(var)
        function.add_code(f"#pragma HLS INTERFACE axis port={self.get_stream_name(self.onnx_node.output[0])}")
    ...
    def append_call_write_output_to_file(self, model, function: cpp_function) -> cpp_function:
        """
        ...
        function.add_code(
            ...
            npy_read.generate_call(
                ...
                self.get_file_name(self.onnx_node.output[0]),
                self.get_stream_name(self.onnx_node.output[0]),
    ...
    def append_variable_declaration(self, function: cpp_function) -> cpp_function:
        ...
        stream_var = cpp_variable(
            self.get_stream_name(self.onnx_node.output[0]),
            f"hls::stream<ap_axiu<{output_bitwidth}, 0, 0, 0>>",
        )
