...
def throughputILP(layers_info, worst_index, valid_par_solutions, NUM_DSP, NUM_PORTS, packing=True, prj_root="/tmp"):
    ...
    for layer in layers_info_unmerged:
        ...
        for single_par in layer_par:
            ...
            if (packing):
                if (layer["bits"] == 8) and (single_par[1] % 2 == 0 or single_par[2] % 2 == 0):
                    dsp_used = np.prod(single_par) * layer["kernel"] // 2
                elif (layer["bits"] == 4) and (single_par[1] % 2 == 0 and single_par[2] % 2 == 0):
...
def parallel_ops_number(io_dict, packing=True, board="ULTRA96v2", prj_root="/tmp"):
    ...
    with open(f"{prj_root}/{board}_par.rpt", "w") as f:
        ...
        for layer in layers_info:
            ...
            if packing and (ich_ops % 2 == 0 or ow_ops % 2 == 0):
                dsp = dsp // 2
