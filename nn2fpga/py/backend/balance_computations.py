        ...
        if node_info["type"] in par_layers:
            ...
            if node_info["type"] == "conv":
                ...
                if (merge_1x1):
                    ...
                    weight_bits.append(
                        node_info["merge_node"]["weight_quant"]["bits"])
                    act_bits.append(
                        node_info["merge_node"]["input_quant"]["bits"])
            else:
...
def generate_architectures(layers_info, NUM_DSP):
    ...
    for layer in layers_info:
        ...
        op_per_dsp, _ = packing_feature(
            (layer["weight_bits"], layer["act_bits"]), (max_och_par, max_ich_par, max_ow_par))
...
def parallelismILP(layers_info, valid_par_solutions, NUM_DSP, NUM_PORTS, prj_root="/tmp"):
    ...
    for par_sol, layer in zip(valid_par_solutions, layers_info):
        ...
        for single_par in par_sol:
            valid_iter_linebuffer[-1].append(layer_iter //
                                             (single_par[1] * single_par[2]))
    ...
    for layer in layers_info_unmerged:
        ...
        for single_par in layer_par:
            op_per_dsp, _ = packing_feature(
                (layer["weight_bits"], layer["act_bits"]), single_par)
    ...
    for layer in layers_info_unmerged:
        ...
        for single_par in layer_par:
            bram_used = compute_bram_layer(
                layer["weight_bits"], n_weights, np.prod(single_par[:2]) * layer["kernel"])
    ...
    for layer_index in [x["index"] for x in layers_info]:
        ...
        prob += (
            pulp.lpDot(
                layer_binary_variables[layer_index].values(), ones) == 1,
...
def resourceILP(layers_info, model_II, valid_par_solutions, parallel_op, NUM_DSP, NUM_PORTS, prj_root="/tmp"):
    ...
    for layer, layer_par in zip(layers_info, clamped_valid_par_solutions):
        ...
        for single_par in layer_par:
            valid_iter_solutions[-1].append(layer_iter // np.prod(single_par))
            valid_iter_linebuffer[-1].append(line_iter //
                                             (single_par[1] * single_par[2]))
    ...
    for i, solution_set in enumerate(valid_par_solutions):
        ...
        for combination in solution_set:
            ...
            if (tot_par == chosen_par and ow_par == chosen_ow):
                ...
                clamped_valid_par_solutions[i].append(combination)
                valid_tot_par_solutions[i].append(np.prod(combination))
                valid_dist_par_solutions[i].append(
                    abs(combination[0] - combination[1]))
...
def opt_steps(layers_info, parallel_op, valid_par_solutions, prj_root="/tmp"):
    ...
    for layer in layers_info[1:]:
        ...
        if (par_prev_out % par_in != 0 and par_in % par_prev_out != 0):
            print(
                f"Error: och_ops i -> {par_prev_out} % ich_ops i+1 -> {par_in} != 0, using {find_common_mult(par_prev_out, par_in)}")
            ...
            for i, combination in enumerate(clamped_valid_par_solutions[layer['index']]):
                ...
                tot_mismatch_after += find_common_mult(
                    par_prev_out, par_in) - max(par_prev_out, par_in)
        par_prev = par
...
def print_report(layers_info, layer_par, n_variables, n_constraints, model_II, time_spent, generate_report_file, prj_root="/tmp"):
    with open(generate_report_file, "a+") as f:
        ...
        # header row
        header = ["Layer name", "ICH", "OCH", "OW", "ich_ops",
                  "och_ops", "ow_ops", "DSPs", "PORTs", "Iter"]
        ...
        for layer in layers_info:
            ...
            op_per_dsp, _ = packing_feature(
                (layer["weight_bits"][0], layer["act_bits"][0]), layer_par[layer['name']])
            pack = str(op_per_dsp)
            dsp = dsp // op_per_dsp
            ...
            port = compute_bram_layer(
                bits, weights,  och_ops * ich_ops * layer["kernel"])
            PORTs += port
            DSPs += dsp
            ...
            if layer["merge_1x1"]:
                ...
                op_per_dsp, _ = packing_feature(
                    (layer["weight_bits"][1], layer["act_bits"][1]), layer_par[layer['name']])
                pack = str(op_per_dsp)
                dsp = dsp // op_per_dsp
...
def write_parallelism(io_dict, model, parallel_ops):
    ...
    for name, node in io_dict.items():
        if "ops" in node:
            ...
            io_dict[input_node_name]["ops_out"] = find_common_mult(
                ops_out, line_ops)
            print(
                f"\tAssigning to node {input_node_name} ops_out = {io_dict[input_node_name]['ops_out']} as mcm between {ops_out} and {line_ops}")
    ...
    for name, node in io_dict.items():
        if node["type"] in line_buffer_layers:
            if (node["in_ops"] % node["line_ops"]) != 0:
                ...
                node["adjust_ops"] = find_common_mult(
                    node["in_ops"], node["line_ops"])
                print(
                    f"\tAdjusting line buffer for {name} over channels: {node['in_ops']} -> {node['adjust_ops']} -> {node['line_ops']} (in_ops -> adjust_ops -> line_ops)")
            else:
    ...
    for name, node in io_dict.items():
        if "ow_ops" in node:
            ...
            if io_dict[output_node_name]["type"] != "consume":
                ...
                print(
                    f"Considering {name} -> {output_node_name}: {node['ow_ops']} -> {next_node['ow_ops']}")
                ...
                if (node["ow_ops"] > next_node["ow_ops"]):
                    ...
                    if (node['ow_ops'] % next_node['ow_ops'] != 0):
                        ...
                        mcm_ow_ops = find_common_mult(
                            node['ow_ops'], next_node['ow_ops'])
                        if (mcm_ow_ops > node['ow']):
                            print(
                                f"Error: {name} -> {output_node_name} ow_ops_out {node['ow_ops_out']} > ow {node['ow']}")
                            exit(-1)
                    ...
                    print(
                        f"Adjusting {name} -> {output_node_name}: {node['ow_ops']} -> {node['ow_ops_out']} -> {next_node['ow_ops_in']} -> {next_node['ow_ops']}")
    ...
    for name, node in io_dict.items():
        if node["type"] == "pool":
            ...
            if (node["in_ops"] < node["ops_out"] or node["in_ops"] % node["ops_out"] != 0):
                node["ops"] = find_max_commond_div(
                    node["in_ops"], node["ops_out"])
                print(
                    f"\tAdjusting pool layer {name} over channels: {node['in_ops']} -> {node['ops']} -> {node['ops_out']} (in_ops -> ops -> ops_out)")
                ...
                if (io_dict[output_node_name]["in_ops"] > node["ops"]):
                    ...
                    print(
                        f"\tAdjusting line buffer for {output_node_name} over channels: {io_dict[output_node_name]['in_ops']} -> {io_dict[output_node_name]['adjust_ops']} -> {io_dict[output_node_name]['line_ops']} (in_ops -> adjust_ops -> line_ops)")
                else:
                    io_dict[output_node_name]["line_ops"] = io_dict[output_node_name]["in_ops"]
            ...
            if node["add"]:
                ...
                if (node["add_ops"] % node["ops"] != 0) or (node["ow_ops"] != ow_ops):
                    ...
                    if ow_ops % node["ow_ops"] != 0 and node["ow_ops"] % ow_ops != 0:
                        print(
                            f"Error: {name} ow_ops ({node['ow_ops']}) is not a multiple/divisor of ow_ops_out of {add_node_name} ({ow_ops})")

                    print(
                        f"\tAdjusting add stream for {name} over channels: {node['add_ops']} -> {node['adjust_add_ops']} -> {node['ops']} (add_ops -> adjust_add_ops -> ops)")
                    print(
                        f"\tAdjusting add stream for {name} over width: {ow_ops} -> {node['adjust_add_ow_ops_in']} -> {node['ow_ops']} (ow_ops_out -> adjust_add_ow_ops_in -> ow_ops)")
                else:
    ...
    for name, node in io_dict.items():
        if node["type"] == "conv":
            _, packing = packing_feature((node["weight_quant"]["bits"], node["input_quant"]["bits"]), [
                                         node["ops"], node["ich_ops"], node["ow_ops"]])
            ...
            if io_dict[output_node_name]["type"] != "consume":
                ...
                if node["type"] == "conv" and node["depth"] == False:
                    if (node["ops"] > node["ops_out"]):
                        print(
                            f"Error: {name} ops_out ({node['ops_out']}) is smaller than ops ({node['ops']}).")
                        return False

                    if (node["ops_out"] % node["ops"] != 0):
                        print(
                            f"Error: {name} ops_out ({node['ops_out']}) is not a multiple of ops ({node['ops']}).")
                        return False

                    if (node["ow_ops"] > node["ow_ops_out"]):
                        print(
                            f"Error: {name} ow_ops_out ({node['ow_ops_out']}) is smaller than ow_ops ({node['ow_ops']}).")
                        return False

                    if (node["ow_ops_out"] % node["ow_ops"] != 0):
                        print(
                            f"Error: {name} ow_ops ({node['ow_ops']}) is not a multiple of ow_ops_out ({node['ow_ops_out']}).")
                        return False
                    ...
                    if node["add"]:
                        if "adjust_add" in node and node["adjust_add"]:
                            if (node["adjust_add_ops"] % node["ops"] != 0):
                                print(
                                    f"Error: {name} adjust_add_ops ({node['adjust_add_ops']}) is not a multiple of ops ({node['ops']}).")
                                return False

                            if (node["adjust_add_ow_ops_in"] % node["ow_ops"] != 0 and node["ow_ops"] % node["adjust_add_ow_ops_in"] != 0):
                                print(
                                    f"Error: {name} adjust_add_ow_ops ({node['adjust_add_ow_ops_in']}) is not multiple of ow_ops ({node['ow_ops']}) and viceversa.")
                                return False
                        else:
                            if (node["add_ops"] % node["ops"] != 0):
                                print(
                                    f"Error: {name} add_ops ({node['add_ops']}) is not a multiple of ops ({node['ops']}).")
                                return False
                ...
                if node["type"] == "conv" and node["depth"] == True:
                    if (node["ops_out"] < node["ich_ops"]):
                        print(
                            f"Error: {name} ops_out ({node['ops_out']}) is smaller than ich_ops ({node['ich_ops']}).")
                        return False

                    if (node["ops_out"] % node["ich_ops"] != 0):
                        print(
                            f"Error: {name} ops_out ({node['ops_out']}) is not a multiple of ich_ops ({node['ich_ops']}).")
                        return False
                    ...
                    if (node["ow_ops_out"] % node["ow_ops"] != 0):
                        print(
                            f"Error: {name} ow_ops_out ({node['ow_ops_out']}) is not a multiple of ow_ops ({node['ow_ops']}).")
                        return False
                ...
                if node["type"] == "pool":
                    if (node["in_ops"] % node["ops"] != 0):
                        print(
                            f"Error: {name} in_ops ({node['in_ops']}) is not a multiple of ops ({node['ops']}).")
                        return False

                    if (node["in_ops"] < node["ops"]):
                        print(
                            f"Error: {name} in_ops ({node['in_ops']}) is smaller than ops ({node['ops']}).")
                        return False
                ...
                if "bandwidth_adjust" in node:
                    if (node["in_ops"] % node["adjust_ops"] != 0):
                        print(
                            f"Error: bandwidth adjust before {name} in_ops ({node['in_ops']}) is not a multiple of adjust_ops ({node['adjust_ops']}).")
                        return False

                    if (node["ow_ops"] % node["ow_ops_in"] != 0 and node["ow_ops_in"] % node["ow_ops"] != 0):
                        print(
                            f"Error: bandwidth adjust before {name} ow_ops ({node['ow_ops']}) not multiple of ow_ops_in ({node['ow_ops_in']}) and viceversa. ")
                        return False
                ...
                if "in_ops" in node and "line_ops" in node:
                    if (node["in_ops"] % node["line_ops"] != 0):
                        print(
                            f"Error: line buffer before {name} in_ops ({node['in_ops']}) is not a multiple of line_ops ({node['line_ops']}).")
                        return False
...
def ilp(io_dict, off_chip_storage, model, file_name, board="ULTRA96v2", generate_report_file="tmp.rpt", prj_root="/tmp"):
    ...
    layer_par, model_II, n_variables, n_constraints, time_spent = parallelismILP(
        layers_info, valid_par_solutions, NUM_DSP, NUM_PORTS, prj_root=prj_root)
    layer_par = resourceILP(layers_info, model_II, valid_par_solutions,
                            layer_par, NUM_DSP, NUM_PORTS, prj_root=prj_root)
    layer_par = balanceILP(layers_info, model_II, valid_par_solutions,
                           layer_par, NUM_PORTS, prj_root=prj_root)
    layer_par = opt_steps(layers_info, layer_par,
                          valid_par_solutions, prj_root=prj_root)
    io_dict = write_parallelism(io_dict, model, layer_par)
    print_report(layers_info, layer_par, n_variables, n_constraints,
                 model_II, time_spent, generate_report_file, prj_root=prj_root)
