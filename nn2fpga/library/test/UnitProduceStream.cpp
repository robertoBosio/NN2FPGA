...
bool test_step() {
  ...
  for (size_t i = 0;
    ...
    actor_status = producer.step(in_stream, out_stream);
  }
...
template <typename TInputStruct, typename TInput, typename TOutputStruct,
          typename TOutput, size_t DATA_PER_WORD, size_t BITS_PER_DATA,
          size_t IN_HEIGHT, size_t IN_WIDTH, size_t OUT_CH, size_t OUT_W_PAR,
          size_t OUT_CH_PAR, size_t PIPELINE_DEPTH>
bool test_step_pipelined() {
  ...
  ProduceStream<TInputStruct, TInput, TOutputStruct, TOutput, TruncQuantizer,
                DATA_PER_WORD, BITS_PER_DATA, IN_HEIGHT, IN_WIDTH, OUT_CH,
                OUT_W_PAR, OUT_CH_PAR, PIPELINE_DEPTH>
      producer;
  ...
  if (DATA_PER_WORD > OUT_CH_PAR * OUT_W_PAR) {
    // If we have more data per word than can be processed in parallel,
    // we need to check that the pipeline does not stall if there is no input,
    // thus the firing index proceeds correctly.
    TInputStruct input_struct;
    input_struct.data = 0;     // Fill with zeros
    input_struct.last = false; // Not the last input
    in_stream.write(input_struct);

    // Step through the producer
    for (size_t i = 0; i < DATA_PER_WORD / (OUT_CH_PAR * OUT_W_PAR); i++) {
      actor_status = producer.step(in_stream, out_stream);
      flag &= actor_status.size() == i + 1 &&
              actor_status.get_current_index() == i + 1;
    }

    // Flush the pipeline
    for (size_t i = 0; i < PIPELINE_DEPTH; i++) {
      actor_status = producer.step(in_stream, out_stream);
      flag &= actor_status.get_current_index() ==
              DATA_PER_WORD / (OUT_CH_PAR * OUT_W_PAR);
    }

    // Check not empty output streams
    TOutputStruct output_struct;
    for (size_t w_par = 0; w_par < OUT_W_PAR; w_par++) {
      flag &= out_stream[w_par].empty() == false; // Should have output
    }
