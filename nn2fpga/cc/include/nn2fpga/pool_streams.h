...
void pool_op(hls::stream<t_input> &i_data, hls::stream<t_output> &o_data) {
...
#pragma HLS stream variable = s_acc_stream depth = 2 type = fifo
  ...
  #pragma HLS pipeline style = stp
      for (auto s_index = 0; s_index < c_index; s_index++) {
        if (s_index == 0) s_acc_buff = c_quant;
...
void pool_kernel(hls::stream<t_input> &i_data, hls::stream<t_output> &o_data) {
#pragma HLS inline
...
template <class t_input, class t_output, class t_acc, int c_ich, int c_och,
          ...
          >
void pool_streams(hls::stream<t_input> &i_data, hls::stream<t_output> &o_data) {
#pragma HLS inline
